...
- **Global Hotkey**: Single key, double-tap, combo, or hold activation
- **Auto-Paste**: Automatically paste polished text into the active application
- **Clipboard Behavior**: Keep text in clipboard always, or only when paste fails
...
This creates a draft release with Windows (.msi/.exe), macOS (.dmg), and Linux (.deb/.AppImage) installers.
...
- **全域快捷鍵**：支援單一按鍵、連按兩下、組合鍵、長按觸發
- **自動貼上**：自動將潤飾後的文字貼到目前的應用程式
- **剪貼簿行為**：可選擇永遠保留文字在剪貼簿，或僅在貼上失敗時保留
...
將自動建立包含 Windows（.msi/.exe）、macOS（.dmg）和 Linux（.deb/.AppImage）安裝程式的草稿版本。
